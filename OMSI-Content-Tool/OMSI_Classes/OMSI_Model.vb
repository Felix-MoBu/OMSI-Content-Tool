...
Public Class OMSI_Model
    ...
    Public ScriptTextures As New List(Of OMSI_ScriptTexture)

    Public TextTexturen As New List(Of OMSI_TextTexture)
            ...
            For linect = 0 To allLines.Count - 1
                If Trim(allLines(linect)) <> "" Then
                    Select Case allLines(linect)
                        Case "[VFDmaxmin]"
                            vfdBox = New OMSI_BBox(toSingle(allLines(linect + 1)), toSingle(allLines(linect + 2)), toSingle(allLines(linect + 3)), toSingle(allLines(linect + 4)), toSingle(allLines(linect + 5)), toSingle(allLines(linect + 6)))
                            linect += 6
                        ...
                        Case "[texttexture_enh]"
                            Dim t_enh As New OMSI_TextTexture With {
                                .enh = True,
                        ...
                        Case "[texttexture]"
                            Dim t_tex As New OMSI_TextTexture With {
                                .index = text_tex_index,
                                .name = allLines(linect + 1),
                                .font = allLines(linect + 2),
                                .breite = allLines(linect + 3),
                        ...
                        Case "[scripttexture]"
                            Dim newST As New OMSI_ScriptTexture
