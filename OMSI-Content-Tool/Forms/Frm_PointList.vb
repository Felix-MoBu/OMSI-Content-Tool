...
Public Class Frm_PointList
    Private Sub Frm_PointList_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        For i = 1 To 20
            ...
            If i < Frm_Main.getProj.ProjDataBase.storedPointNames.count Then
                newTB.Text = Frm_Main.getProj.ProjDataBase.storedPointNames(i)
            End If
            Me.Controls.Add(newTB)

            Dim newPS As New PointSelector
            copyProps(PS0, newPS, i)
            If i < Frm_Main.getProj.ProjDataBase.storedPoints.count Then
                newPS.Point = Frm_Main.getProj.ProjDataBase.storedPoints(i)
            End If
            ...
            If IsNumeric(control.tag) Then
                If control.GetType Is TB0.GetType Then
                    If control.text <> "" Then
                        For i As Integer = Frm_Main.getProj.ProjDataBase.storedPointNames.count - 1 To control.tag
                            Frm_Main.getProj.ProjDataBase.storedPointNames.add("")
                        Next
                        Frm_Main.getProj.ProjDataBase.storedPointNames(control.tag) = control.text
                    End If
                ElseIf control.GetType Is PS0.GetType Then
                    If Not control.point = New Point3D Then
                        For i As Integer = Frm_Main.getProj.ProjDataBase.storedPoints.count - 1 To control.tag
                            Frm_Main.getProj.ProjDataBase.storedPoints.add(New Point3D)
                        Next
                        Frm_Main.getProj.ProjDataBase.storedPoints(control.tag) = control.Point
                    End If
                End If
            End If
        Next
        Frm_Main.getProj.ProjDataBase.Save()
    End Sub
End Class